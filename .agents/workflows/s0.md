...
### 步骤 1: 提取项目名称与描述（含痛点与成功定义）

**AI 执行指引**：
1. 从用户描述中提取简洁项目名称（2-6 个字），以「行业+系统类型」命名（如「制造业ERP」「餐饮SaaS」）
2. 生成一句话描述（30-80 字），涵盖：做什么 + 给谁用 + 核心价值
3. 展示给用户确认，允许调整
4. **主动追问痛点**：「这个系统主要解决什么痛点？」—— 用户回答后填入 `problem.pain_points`
5. **可选追问 KPI**：「有没有可量化的成功标准？」—— 用户可以只说一句话，也可以跳过

**交互节点**：
- 🗣️ 展示提取的名称和描述 → 用户确认或修改
- 🗣️ 自然地追问痛点和成功标准（不要像填表格，要像聊天）

**异常处理**：
- 用户描述过于模糊（<10 字）→ 引导性提问：「这个系统主要服务哪类用户？核心要解决什么问题？」
- 用户描述过于宽泛 → 提示聚焦：「范围偏大，建议缩窪到一个核心场景先推进，后续迭代扩展」
- 用户跳过痛点/KPI → 允许，对应字段留空
...
|----------|---------------------|
| 步骤 1 | `name`, `description`, `problem`, `success_metrics` |
| 步骤 2 | `target_modules` |
| 步骤 3 | `constraints.budget`, `constraints.timeline`, `constraints.team_size`, `known_risks` |
